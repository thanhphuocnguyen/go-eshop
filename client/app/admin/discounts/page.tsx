...
export default function DiscountsPage() {
  ...
  const filteredDiscounts = mockDiscounts.filter(discount => {
    ...
    if (activeFilter === 'active') {
      return matchesSearch && discount.isActive;
              ...
              <tr key={discount.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <div className="text-xs text-gray-500">
                    {discount.minPurchaseAmount ? `Min: $${discount.minPurchaseAmount}` : 'No minimum'}
                  </div>
                  <div className="text-xs text-gray-500">
                    {discount.maxDiscountAmount ? `Max: $${discount.maxDiscountAmount}` : 'No maximum'}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  {discount.usageLimit && (
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div 
                        ...
                        style={{ width: `${Math.min(100, (discount.usedCount / discount.usageLimit) * 100)}%` }}
                      ></div>
                    </div>
                  )}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                  <div className="flex justify-center space-x-2">
                    <Link href={`/admin/discounts/${discount.id}`} className="text-gray-500 hover:text-gray-900">
                    ...
                    <Link href={`/admin/discounts/${discount.id}/edit`} className="text-indigo-600 hover:text-indigo-900">
