    ...
    <div className='h-full overflow-auto my-auto p-6'>
      ...
      {sortedBrands.length === 0 ? (
        ...
        <div className="bg-white overflow-hidden shadow rounded-lg">
          <TableContainer
            header={['', 'Name', 'Description', 'Status', 'Products', 'Created At', 'Actions']}
          >
            {sortedBrands.map((brand) => (
              <TableRow key={brand.id}>
                <TableCell className="w-16">
                  {brand.image_url ? (
                    ...
                    <div className="h-10 w-10 rounded-md bg-gray-200 flex items-center justify-center">
                      <span className="text-gray-400 text-xs">{brand.name.charAt(0).toUpperCase()}</span>
                    </div>
                  )}
                </TableCell>
                <TableCellHead>
                ...
                <TableCell>
                  <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${brand.published ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                ...
                <TableCell>{dayjs(brand.created_at).format('YYYY/MM/DD')}</TableCell>
                <TableCell>
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  Showing <span className="font-medium">{sortedBrands.length}</span> brands
                </p>
              </div>
              <div>
