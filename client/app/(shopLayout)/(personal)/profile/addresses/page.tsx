...
import { Button, Dialog, DialogPanel, DialogTitle, Fieldset, Label } from '@headlessui/react';
...
export default function AddressesPage() {
    ...
    try {
      if (selectedAddress) {
        ...
        const response = await apiFetch<GenericResponse<AddressModel>>(
          PUBLIC_API_PATHS.USER_ADDRESS.replace(':id', selectedAddress.id.toString()),
          {
            method: 'PUT',
            body: data,
          }
  ...
  const handleSetDefaultAddress = async (addressId: number) => {
    try {
      const response = await apiFetch<GenericResponse<AddressModel>>(
        PUBLIC_API_PATHS.USER_ADDRESS_DEFAULT.replace(':id', addressId.toString()),
        {
          method: 'PATCH',
        }
      );
      ...
      {user?.addresses && user.addresses.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {user.addresses.map((address) => (
            <div key={address.id} className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm hover:shadow-md transition-shadow">
              {address.default && (
