...
FROM 
  ...
  JOIN (
    ...
    VALUES 
      ...
      (
        'moderator', 'products', true, true, 
        false
      ), 
      (
        'moderator', 'categories', true, true, 
        false
      ), 
      (
        'moderator', 'brands', true, true, 
        false
      ), 
      (
        'moderator', 'collections', true, 
        true, false
      ), 
      (
        'moderator', 'ratings', true, true, 
        true
      ), 
      (
        'moderator', 'orders', true, false, 
        false
      ), 
      (
        'moderator', 'users', true, false, 
        false
      )
  ) AS perm(
    role_code, module, read_perm, write_perm, 
    execute_perm
  ) ON r.code = perm.role_code ON CONFLICT (role_id, module) DO NOTHING;
...
CREATE TABLE IF NOT EXISTS payment_methods (
  ...
  description TEXT, 
  is_active BOOLEAN NOT NULL DEFAULT true, 
  gateway_supported VARCHAR(100), 
  -- e.g., 'stripe', 'paypal', null for COD
  icon_url TEXT, 
  requires_account BOOLEAN NOT NULL DEFAULT false, 
  -- Whether this method requires user account setup
...
INSERT INTO payment_methods (
  code, name, description, gateway_supported, 
  requires_account, min_amount, max_amount, 
...
VALUES 
  (
    ...
    'stripe', false, 1.00, 10000.00, 0.0290, 
    0.30, ARRAY[ 'USD', 'EUR', 'GBP' ], 
    ARRAY[ 'US', 'CA', 'GB', 'EU' ]
  ), 
  (
    'debit_card', 'Debit Card', 'Debit card payment method', 
    'stripe', false, 1.00, 5000.00, 0.0290, 
    0.30, ARRAY[ 'USD', 'EUR', 'GBP' ], 
    ARRAY[ 'US', 'CA', 'GB', 'EU' ]
  ), 
  (
    'paypal', 'PayPal', 'PayPal payment method', 
    'paypal', false, 1.00, 10000.00, 0.0349, 
    0.49, ARRAY[ 'USD', 'EUR', 'GBP', 'CAD' ], 
    ARRAY[ 'US', 'CA', 'GB', 'EU', 'AU' ]
  ), 
  (
    'stripe', 'Stripe', 'Stripe payment method', 
    'stripe', false, 0.50, 999999.99, 
    0.0290, 0.30, ARRAY[ 'USD', 'EUR', 
    'GBP' ], ARRAY[ 'US', 'CA', 'GB', 'EU' ]
  ), 
  (
  ...
  (
    ...
    null, false, 10.00, 50000.00, 0.0000, 
    0.00, ARRAY[ 'USD', 'EUR' ], ARRAY[ 'US', 
    'EU' ]
  ), 
  (
    'cod', 'Cash on Delivery', 'Cash on delivery payment method', 
    null, false, 5.00, 500.00, 0.0000, 
    2.00, ARRAY[ 'USD' ], ARRAY[ 'US' ]
  ) ON CONFLICT (code) DO NOTHING;
...
CREATE TABLE email_verifications (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW (), 
  expired_at TIMESTAMPTZ NOT NULL DEFAULT (
    NOW () + interval '1 day'
  )
);
...
CREATE TABLE products (
  ...
  discount_percentage SMALLINT DEFAULT 0 CHECK (
...
CREATE TABLE discount_rules (
  ...
  -- 'product', 'category', 'customer_segment'
...
CREATE TABLE product_ratings (
  ...
  product_id UUID NOT NULL REFERENCES products(id) ON DELETE CASCADE, 
  user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE, 
  order_item_id UUID REFERENCES order_items(id) ON DELETE 
  SET 
    NULL, 
...
CREATE TABLE IF NOT EXISTS shipping_rate_conditions (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(), 
...
ADD 
  CONSTRAINT fk_orders_shipping_rate FOREIGN KEY (shipping_rate_id) REFERENCES shipping_rates(id);
-- Create functions and triggers
-- Function to set default user role
CREATE 
OR REPLACE FUNCTION set_default_user_role() RETURNS TRIGGER AS $$ BEGIN -- If role_id is not provided, set it to 'user' role
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trigger_set_default_user_role BEFORE INSERT ON users FOR EACH ROW EXECUTE FUNCTION set_default_user_role();
CREATE 
OR REPLACE FUNCTION update_discount_updated_at() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = NOW();
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER update_discount_timestamp BEFORE 
UPDATE 
  ON discounts FOR EACH ROW EXECUTE FUNCTION update_discount_updated_at();
-- Create function to update product average ratings
CREATE 
OR REPLACE FUNCTION update_product_avg_rating() RETURNS TRIGGER AS $$ DECLARE target_product_id UUID;
BEGIN -- Determine the product_id based on the operation
IF TG_OP = 'DELETE' THEN target_product_id := OLD.product_id;
ELSE target_product_id := NEW.product_id;
END IF;
...
AFTER 
  INSERT ON product_ratings FOR EACH ROW EXECUTE FUNCTION update_product_avg_rating();
CREATE TRIGGER after_rating_update 
AFTER 
UPDATE 
  ON product_ratings FOR EACH ROW WHEN (
    OLD.rating != NEW.rating 
    OR OLD.is_visible != NEW.is_visible 
    OR OLD.is_approved != NEW.is_approved
  ) EXECUTE FUNCTION update_product_avg_rating();
CREATE TRIGGER after_rating_delete 
AFTER 
  DELETE ON product_ratings FOR EACH ROW EXECUTE FUNCTION update_product_avg_rating();
...
CREATE INDEX IF NOT EXISTS idx_discounts_active_dates ON discounts(valid_from, valid_until) 
WHERE 
  is_active = true;
...
CREATE INDEX idx_shipping_rates_method_zone ON shipping_rates(
  shipping_method_id, shipping_zone_id
);
